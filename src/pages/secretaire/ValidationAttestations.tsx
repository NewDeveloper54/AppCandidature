...
export default function ValidationAttestations() {
  ...
  const fetchPending = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/secretaire/attestations-pendantes"
      );
      const data = await res.json();
      console.log("voir les donées :", data);
  ...
  const handleDecision = async (
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/secretaire/valider-attestation",
        {
          method: "PUT",
    ...
    <div className="space-y-4">
      ...
      {attestations.length === 0 ? (
        ...
        attestations.map((att) => (
          <Card
            key={att.id_utilisateur}
            className="bg-neutral-900/50 border-white/5 hover:border-white/10 transition-all shadow-xl rounded-2xl"
          >
            <CardContent className="flex flex-col md:flex-row items-center justify-between p-6 gap-6">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                <Button
                  ...
                  variant="outline"
                  className="border-white/10  hover:bg-white/50 gap-2 h-11"
                >
                  <Eye  size={18} /> Voir
                </Button>
